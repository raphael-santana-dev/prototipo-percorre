<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-900">Gerenciamento de Features</h1>
    </div>

    <div class="p-6 bg-white border border-gray-100 shadow-sm rounded-xl">
        <h2 class="text-lg font-medium text-gray-900">Cadastrar Nova Feature</h2>
        <form wire:submit="addFeature" class="flex flex-wrap items-end gap-4 mt-4">
            <div class="w-full md:w-1/4">
                <label for="module" class="block text-sm font-medium text-gray-700">Módulo (ex: curso)</label>
                <input type="text" id="module" wire:model="module" class="w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                @error('module') <span class="text-xs text-red-600">{{ $message }}</span> @enderror
            </div>
            <div class="w-full md:w-1/4">
                <label class="block text-sm font-semibold text-gray-800">Ação/Recurso (ex: criar)</label>
                <input type="text" id="name" wire:model="action" class="w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
    ...
    <div class="overflow-hidden bg-white border border-gray-100 shadow-sm rounded-xl">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($featuresByModule as $module => $features)
                    <!-- Linha Separadora de Módulo -->
                    <tr class="bg-gray-100">
                        <td colspan="4" class="px-6 py-2 text-sm font-bold text-gray-800 uppercase">
                            Módulo: {{ $module }}
                        </td>
                    </tr>
                    <!-- Features do Módulo -->
                    @foreach($features as $feature)
                        <tr>
                            ...
                            <td class="px-6 py-4 text-sm font-medium text-right whitespace-nowrap">
                                <button wire:click="toggle('{{ $feature->name }}', {{ $feature->is_active ? 'true' : 'false' }})" class="{{ $feature->is_active ? 'text-red-600 hover:text-red-900' : 'text-green-600 hover:text-green-900' }} font-semibold">
