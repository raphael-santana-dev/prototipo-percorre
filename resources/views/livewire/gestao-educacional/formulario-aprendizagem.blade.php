@php
    ...
    $cardClass = $isTranslucent ? 'bg-white/80 backdrop-blur-md shadow-2xl dark:bg-gray-900/80' : 'bg-white shadow-xl dark:bg-gray-900';
    $textoForm = $isTranslucent ? 'text-gray-900 drop-shadow-sm dark:text-white' : 'text-gray-900 dark:text-white';
@endphp
...
<div class="min-h-screen flex flex-col relative w-full font-sans bg-transparent">
    ...
    <div class="relative z-10 w-full {{ $formWidth }} mx-auto py-12 px-4 sm:px-6 flex-1 flex flex-col justify-center">
        ...
        @if(!$podeResponder)
            <div class="{{ $cardClass }} p-10 md:p-16 rounded-xl text-center border-t-4 border-yellow-500 transition-all duration-300">
                <div class="inline-flex items-center justify-center w-20 h-20 rounded-full bg-yellow-50 text-yellow-600 mb-6 shadow-sm border border-yellow-100 dark:bg-yellow-900/30">
                ...
                <button type="button" onclick="window.history.back()" class="mt-8 inline-block bg-gray-900 hover:bg-black text-white font-bold py-3 px-8 rounded-lg shadow-sm transition">
        ...
        @else
            <div class="{{ $cardClass }} p-8 md:p-12 rounded-xl border-t-4 border-purpura-600 transition-all duration-300">
                <div class="mb-8 pb-6 border-b border-gray-200 dark:border-gray-700">
                    <span class="inline-block px-2.5 py-1 bg-purpura-50 text-purpura-700 dark:bg-purpura-900/30 text-xs font-bold rounded-md uppercase tracking-wider mb-2">
                        Aprendiz: {{ $aluno->name }}
                    </span>
                ...
                <div class="mt-10 pt-6 border-t border-gray-200 dark:border-gray-700 flex justify-between items-center">
                    @if($etapaAtual > 1)
                        <button type="button" wire:click="$set('etapaAtual', {{ $etapaAtual - 1 }})" class="text-gray-600 hover:text-purpura-600 font-bold py-2.5 px-4 transition flex items-center gap-2">
                    ...
                    <button type="button" wire:click="salvarEAvancar" class="bg-purpura-600 text-white font-bold py-3 px-8 rounded-lg shadow-md hover:bg-purpura-700 flex items-center gap-2">
                        {{ $etapaAtual === $totalEtapas ? 'Finalizar e Enviar Respostas' : 'Próxima Etapa' }}
