<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-900 flex items-center gap-2 dark:text-white">
            <i class="ph ph-key text-purpura-500"></i> Permissões de Acesso
        </h1>
        <button wire:click="openModal" class="flex items-center gap-2 px-4 py-2 text-white transition-colors rounded-lg bg-purpura-500 hover:bg-purpura-600">
            <i class="ph ph-plus"></i> Novas Permissões
        </button>
    </div>
    ...
    <!-- Listagem Agrupada por Módulos -->
    <div class="space-y-6">
        @forelse($permissionsByModule as $moduleName => $permissionsGroup)
                    ...
                    <tbody class="bg-white divide-y divide-gray-100 dark:bg-gray-800 dark:divide-gray-700">
                        @foreach($permissionsGroup as $permission)
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                ...
                                <!-- Coluna de Ações com Ícones (Padrão Etapa 25) -->
                                <td class="px-6 py-4 text-right whitespace-nowrap w-24">
                                    <div class="flex items-center justify-end gap-2">
                                        <button wire:click="edit({{ $permission->id }})" class="p-2 text-gray-400 transition-colors rounded-lg hover:text-purpura-500 hover:bg-purpura-50 dark:hover:bg-gray-600" title="Editar">
                                            <i class="text-xl ph ph-pencil-simple"></i>
                                        </button>
                                        <button wire:click="delete({{ $permission->id }})" class="p-2 text-gray-400 transition-colors rounded-lg hover:text-red-500 hover:bg-red-50 dark:hover:bg-gray-600" title="Excluir" onclick="confirm('Excluir esta permissão pode quebrar o acesso ao sistema. Continuar?') || event.stopImmediatePropagation()">
    ...
    @if($showModal)
        <div class="fixed inset-0 z-50 overflow-y-auto">
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <!-- Modal largo (max-w-4xl) para caber as colunas horizontais confortavelmente -->
                <div class="relative z-10 inline-block px-4 pt-5 pb-4 overflow-visible text-left align-bottom transition-all transform bg-white rounded-xl shadow-xl sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full sm:p-6 dark:bg-gray-800">
                    ...
                    <h3 class="mb-4 text-lg font-bold text-gray-900 border-b border-gray-100 pb-2 dark:text-white dark:border-gray-700">
                        {{ $isEditMode ? 'Editar Permissão' : 'Cadastrar Permissões' }}
                    </h3>
                    
                    <form wire:submit="save" class="space-y-4">
                        
                        <!-- Cabeçalho das Colunas (Oculto no mobile) -->
                        ...
                        <!-- Laço de repetição: Cria os inputs baseado no tamanho do array $items -->
                        @foreach($items as $index => $item)
                            <div class="grid grid-cols-1 gap-4 p-4 mb-4 border border-gray-200 rounded-lg md:p-0 md:border-none md:grid-cols-12 dark:border-gray-700 bg-gray-50/50 md:bg-transparent dark:bg-gray-800/50 items-start">
                                ...
                                <div class="md:col-span-3">
                                    ...
                                    <input type="text" wire:model="items.{{ $index }}.module" class="w-full text-sm" placeholder="ex: turno">
                                ...
                                <div class="md:col-span-3">
                                    ...
                                    <input type="text" wire:model="items.{{ $index }}.action" class="w-full text-sm" placeholder="ex: editar">
                                ...
                                <div class="md:col-span-5">
                                    ...
                                    <input type="text" wire:model="items.{{ $index }}.description" class="w-full text-sm" placeholder="O que esta permissão faz?">
                                    @error("items.{$index}.description") <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                                </div>
                                
                                <!-- Botão de Lixeira (Só aparece se houver mais de uma linha e não for edição) -->
                        ...
                        <!-- Botão Adicionar Mais (Oculto na Edição) -->
                        @if(!$isEditMode)
                            <div class="pt-2">
