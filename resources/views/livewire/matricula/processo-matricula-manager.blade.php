<div class="p-6 max-w-7xl mx-auto font-sans relative">
    ...
    <x-page-header title="Acompanhamento de Matrículas" icon="ph ph-folder-user" badge="Portal de Envio">
    ...
    <!-- TABELA NATIVA (Para evitar bloqueios de renderização) -->
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse whitespace-nowrap">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        <th class="p-4 font-bold text-xs text-gray-500 uppercase tracking-wider w-16 text-center">#ID</th>
                        <th class="p-4 font-bold text-xs text-gray-500 uppercase tracking-wider">Candidato / Curso</th>
                        <th class="p-4 font-bold text-xs text-gray-500 uppercase tracking-wider text-center">Progresso</th>
                        <th class="p-4 font-bold text-xs text-gray-500 uppercase tracking-wider text-center">Etapa Atual</th>
                        <th class="p-4 font-bold text-xs text-gray-500 uppercase tracking-wider text-right">Dossiê</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    @forelse($registros as $inscricao)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-4 py-3 text-sm font-bold text-gray-500 text-center">#{{ $inscricao->id }}</td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 text-center">
                                ...
                                <div class="flex flex-col items-center justify-center w-full max-w-[150px] mx-auto">
                                    ...
                                    <div class="w-full bg-gray-200 rounded-full h-2.5 overflow-hidden">
                                        <div class="h-2.5 rounded-full {{ $porcentagem == 100 ? 'bg-green-500' : 'bg-purpura-500' }}" style="width: {{ $porcentagem }}%"></div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-3 text-right">
                                <button wire:click="abrirDossie({{ $inscricao->id }})" class="px-3 py-1.5 bg-white border border-gray-300 hover:bg-gray-50 hover:text-purpura-600 text-gray-700 font-bold text-xs rounded-lg shadow-sm transition inline-flex items-center gap-2">
                        ...
                        @foreach($documentosExigidos as $docReq)
                            ...
                            <div class="bg-white border rounded-xl p-4 shadow-sm {{ $status === 'valido_ia' || $status === 'aprovado_manual' ? 'border-green-200' : 'border-gray-200' }}">
                                <div class="flex justify-between items-start mb-2">
                                    ...
                                    @if($status === 'valido_ia') <span class="text-[10px] bg-green-100 text-green-700 font-bold px-2 py-1 rounded-full"><i class="ph-bold ph-robot"></i> IA Aprovou</span>
                                    @elseif($status === 'aprovado_manual') <span class="text-[10px] bg-green-100 text-green-700 font-bold px-2 py-1 rounded-full"><i class="ph-bold ph-user"></i> Sec. Aprovou</span>
                                    @elseif($status === 'analise_manual' || $status === 'invalido_ia') <span class="text-[10px] bg-yellow-100 text-yellow-700 font-bold px-2 py-1 rounded-full"><i class="ph-bold ph-warning"></i> Validar Manualmente</span>
                                    @else <span class="text-[10px] bg-gray-100 text-gray-500 font-bold px-2 py-1 rounded-full">Pendente</span>
                                    @endif
                                </div>

                                @if($enviado)
                                    <div class="mt-3 bg-gray-100 rounded-lg overflow-hidden border border-gray-200 flex items-center justify-center h-40 relative">
                                        @if($imgBase64)
                                            <a href="{{ $imgBase64 }}" target="_blank" title="Clique para ampliar" class="w-full h-full flex items-center justify-center cursor-zoom-in">
                                                <img src="{{ $imgBase64 }}" class="max-h-full object-contain">
                                            </a>
                                        @else
                                            <span class="text-xs text-gray-400">Erro ao carregar arquivo.</span>
                                        @endif
                                    </div>
                                    <div class="mt-3 flex gap-2">
                                        <button wire:click="aprovarDocumento({{ $enviado->id }})" class="flex-1 text-[11px] font-bold py-1.5 rounded bg-green-50 text-green-700 hover:bg-green-600 hover:text-white transition border border-green-200 flex items-center justify-center gap-1"><i class="ph-bold ph-check"></i> Aprovar</button>
                                        <button wire:click="reprovarDocumento({{ $enviado->id }})" class="flex-1 text-[11px] font-bold py-1.5 rounded bg-red-50 text-red-700 hover:bg-red-600 hover:text-white transition border border-red-200 flex items-center justify-center gap-1"><i class="ph-bold ph-x"></i> Recusar</button>
                                    </div>
                                    @if(in_array($status, ['analise_manual', 'invalido_ia']))
                                        <p class="text-[10px] text-red-600 mt-2 font-medium bg-red-50 p-1.5 rounded">Motivo IA: {{ $enviado->log_ia['motivo_rejeicao'] ?? 'Rejeitado' }}</p>
                                    @endif
                                @else
